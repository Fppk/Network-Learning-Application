...
export function Quiz({ title = "Knowledge Check", questions, onComplete }: QuizProps) {
    ...
    const handleSubmit = () => {
        ...
        if (isCorrect) {
            setScore(score + 1)
            toast.success("Correct!", { description: "Great job." })
        } else {
            toast.error("Incorrect", { description: "Review the explanation." })
        }
        setIsAnswered(true)
    }
                ...
                <CardContent className="flex flex-col items-center space-y-6">
                    ...
                    {percentage >= 80 ? (
                        <Alert className="bg-green-50 dark:bg-green-900/20 border-green-200">
                            ...
                            <AlertDescription>You have mastered this topic.</AlertDescription>
                        </Alert>
                    ) : (
                        <Alert className="bg-orange-50 dark:bg-orange-900/20 border-orange-200">
                            <AlertCircle className="h-4 w-4 text-orange-600" />
                            <AlertTitle>Keep Practicing</AlertTitle>
                            <AlertDescription>Review the material and try again.</AlertDescription>
                        </Alert>
                    )}
                </CardContent>
